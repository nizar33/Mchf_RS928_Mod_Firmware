...
#define		BUTTON_PRESS_DEBOUNCE			73		// NIZZZ  time to debounce normal button press
#define 	BUTTON_HOLD_TIME				1000    // 1000	// time of press-and-hold
...
{
	ulong	button_id;                // ID of button as GPIO bit
    ulong	debounce_time;            // Actual time the button is pressed
    bool	button_pressed;           // Flag to indicate click event
 // bool	button_released;          // Flag to indicate release event
    bool	button_execution_Wait;    // Flag to indicate under process
//  bool	button_continued_pressed; // Flag to indicate that the button had been continued to be pressed during debounce
    bool	debounce_checked;         // Flag to indicate that debounce check was complete
    bool	press_holded;             // Flag to indicate a "press-and-hold" condition
    uchar	dummy;
} KeypadState;

#define	PRESS_HOLD_RELEASE_DELAY_TIME	 10	  //30	// time after button being released before resetting press-and-hold flag
...
#define	PWR_DAMPENING_FACTOR				0.1   ///0.10		// dampening/averaging factor (e.g. amount of "new" reading each time) - for numerical power reading ONLY
//
...
// Volt (DC power) meter
//
#define POWER_SAMPLES_CNT					16  // 32 NIZZ pour lire plus rapidement la tension Alim
//
...
void UiAction_ChangeFrequency_Sub500Hz();
...
void UiAction_Debug_switch() ;      /// NIZZ
...
void UiAction_RAZ_RIT_And_Display();
...
//
#define	DSP_STARTUP_DELAY					700     // 350 // Delay, in 100ths of seconds, after startup, before allowing DSP NR or Notch to be enabled.
...
//
#define TXRX_SWITCH_AUDIO_MUTE_DELAY_DEFAULT     1   // 1  // 10ms Default, in 100ths of a second, that audio will be muted after PTT (key-up/key-down) to prevent "clicks" and "clunks"
#define	TXRX_SWITCH_AUDIO_MUTE_DELAY_MAX	    25   // 25 // Maximum delay, in 100ths of a second, that audio will be muted after PTT (key-up/key-down) to prevent "clicks" and "clunks"
...
void UiAction_Attenuator_switch();  /// NIZZ
