...
void UiSpectrum_CalculateLayout( const bool is_big, const bool scope_enabled, const  bool  wfall_enabled, const UiArea_t* full_ptr, const uint16_t padding )
{
    ...
    slayout.graticule.h = 14;  // 16 NIZZZ
...
void UiSpectrum_ResetSpectrum()
{
	...
	{
		case FLAGS1_SCOPE_ENABLED: 	slayout.graticule.y = slayout.draw.y + slayout.draw.h - slayout.graticule.h;	    break;
		case FLAGS1_WFALL_ENABLED:	slayout.graticule.y = slayout.draw.y + slayout.title.h;		                        break;
		case (FLAGS1_SCOPE_ENABLED |
			 FLAGS1_WFALL_ENABLED): slayout.graticule.y = UiSprectrum_CheckNewGraticulePos( ts.graticulePowerupYpos );  break;
		default: 		                                                                                                break;
	}
}
...
//
#define DB_SCALING_5                        79.057      // 63.2456     // 5dB/division scaling
#define DB_SCALING_7                        63.2456     // 42.1637     // 7.5dB/division scaling
#define DB_SCALING_10                       42.1637     // 31.6228     // 10dB/division scaling
#define DB_SCALING_15                       31.6228     // 21.0819     // 15dB/division scaling
#define DB_SCALING_20                       21.0819     // 15.8114     // 20dB/division scaling
...
static void UiSpectrum_UpdateSpectrumPixelParameters()
{
    ...
    if (ts.cw_lsb != old_cw_lsb || ts.cw_sidetone_freq != old_cw_sidetone_freq || ts.dmod_mode != old_dmod_mode || ts.digital_mode != old_digital_mode || force_update)
    {
        ...
        sd.line_offset = mode_marker_offset[0];
    }
...
// was used on 320x240, we may reactivate that at some point for all resolutions
    ...
    if(slayout.title.h != 0)		//don't draw text bar if there is no space allocated for it
    {
    	...
    	UiLcdHy28_PrintTextCentered(	slayout.title.x,
    ...
    if (is_waterfallmode() == true && ts.waterfall.speed == 0)
    {
        ...
        UiLcdHy28_PrintTextCentered(	slayout.wfall.x,
    ...
    {
        // print "disabled" in the middle of the screen if the waterfall or scope was disabled
        UiLcdHy28_PrintTextCentered(	slayout.scope.x,
...
{
    UiLcdHy28_DrawFullRect ( slayout.full.x,  slayout.full.y + 1,  slayout.full.h - 1 ,  slayout.full.w,  Black);	// Clear screen under spectrum scope by drawing a single, black block (faster with SPI!)
...
static void    UiSpectrum_DrawScope(uint16_t *old_pos, float32_t *fft_new)
{
    ...
    UiMenu_MapColors  ( ts.scope_backgr_BW_colour   , NULL, &clr_scope_fltr  ); //background colour of highlight
    					// j'ai force la couleur bleu
    uint16_t   BWHbgr = ts.scope_backgr_BW_transparence;
    BWHbgr  <<= 8;
    BWHbgr   /= 100;
    ...
    for ( uint16_t idx = 0; idx < SPECTRUM_MAX_MARKER; idx++)
    {
        ...
        {
            if ((sd.marker_line_pos_prev[idx] < slayout.scope.x + slayout.scope.w) && ( sd.marker_line_pos_prev[idx] > slayout.scope.x ))
            {
                ...
                if(is_scope_light)
                {
                    UiLcdHy28_DrawStraightLine( sd.marker_line_pos_prev[idx],	spec_top_y - spec_height_limit,
																				spec_height_limit,
																				LCD_DIR_VERTICAL,
																				clr_bg);
                }
                ...
                {
                    UiSpectrum_ScopeStandard_UpdateVerticalDataLine(	sd.marker_line_pos_prev[idx],
            ...
            if ((marker_line_pos[idx] < slayout.scope.x + slayout.scope.w) && ( marker_line_pos[idx] > slayout.scope.x))
            {

                // draw new line if inside screen limits

                UiLcdHy28_DrawStraightLine( marker_line_pos[idx],	spec_top_y - spec_height_limit,
																	spec_height_limit,
																	LCD_DIR_VERTICAL,
																	sd.scope_centre_grid_colour_active);
    ...
    for ( uint16_t x = slayout.scope.x, idx = 0; ts.ptt_req == false && idx < slayout.scope.w; x++, idx++)
    {
        ...
        if   ( fft_new[idx] < spec_height_limit ) { y_height = fft_new[idx] ;     }  // NIZZZ
        else                                      { y_height = spec_height_limit;  if (prem_fois && (ts.spec_decal < 5)) {  ts.spec_decal++; prem_fois = false;} }
    ...
    if ((prem_fois ) && (ts.spec_decal > 0))   ts.spec_decal--; // s'il n'y a pas de saturation du graphic spectrale on se permet de soulever le graphic
}
...
static void UiSpectrum_InitSpectrumDisplayData()
{
    ...
    {
		case WFALL_HOT_COLD:        wfall_scheme = &waterfall_cold_hot    [0];   break;
		case WFALL_RAINBOW:         wfall_scheme = &waterfall_rainbow     [0];   break;
		case WFALL_BLUE:            wfall_scheme = &waterfall_blue        [0];   break;
		case WFALL_GRAY_INVERSE:    wfall_scheme = &waterfall_grey_inverse[0];   break;
		case WFALL_GRAY:
		default:                    wfall_scheme =  &waterfall_grey       [0];   break;
    }
...
static float32_t  UiSpectrum_Scale_FFT_Value( const float32_t value, float32_t*  min_p )
{
    float32_t sig =  sd.display_offset + log10f_fast(value) * sd.db_scale;  // NIZZ ajout� -3   // take FFT data, do a log10 and multiply it to scale 10dB (fixed)
    // apply "AGC", vertical "sliding" offset (or brightness for waterfall)
    //if (sig < 1) sig =1;
    if (sig < *min_p)   {  *min_p = sig;  }
...
static void UiSpectrum_ScaleFFT ( float32_t  dest[],  float32_t  source[], float32_t*  min_p )
{
    ...
    {
        dest [ sd.spec_len - i - 1 ] = UiSpectrum_Scale_FFT_Value ( source[i + sd.spec_len/2], min_p) - ts.spec_decal;  // NIZZ -7 pour faire baisser le graphic du spectre de qq 8 pixels
...
static void UiSpectrum_ScaleFFT2SpectrumWidth( float32_t samples[], uint16_t from_len, uint16_t to_len )
{
	...
	if ( to_len < from_len)
	{
		const float32_t full_amount = (float32_t)from_len/(float32_t)to_len; // 1.06666
	...
	else
	{
		...
		arm_copy_f32 (samples, Tampon_Tab, 256 );

	 	Tampon_Tab[256] = Tampon_Tab[255] ;


		do
		{
			duree   += 0.8 ;
			if ( duree >   1 )   { duree -= 1.0;  idx_src++; }

	//		if      ( duree < 0.3 ) {  samples [ idx_des ]  =   Tampon_Tab [ idx_src     ];                                      }
	//		else if ( duree < 0.7 ) {  samples [ idx_des ]  = ( Tampon_Tab [ idx_src     ] + Tampon_Tab [ idx_src + 1 ] ) * 0.5; }
	//		else                    {  samples [ idx_des ]  =   Tampon_Tab [ idx_src + 1 ];     								 }

			idx_des++;
			samples [ idx_des ] = Tampon_Tab [ idx_src] + ( Tampon_Tab [ idx_src + 1] - Tampon_Tab [ idx_src ] ) * duree;
		} while (idx_des < 320);
	}
}
/*
 ...
 */
static void UiSpectrum_RedrawSpectrum()  // appele  chaque 10 ms  pour calculer et afficher le  FFT 256/512
										// saisie, calcul, moyennage et affichage du spectre se fait en 7 etapes dans le temps
                                        // environ un affichage chaque 60ms au maximum ( 17 fois /seconde )
{
	...
	static  float32_t  freq_calcc         ;
	static  int32_t    last_cur_center_hz ;
    ...
    switch(sd.state)
    {
		case 0:
				...
				{
					ts.FFT_stand_by--;
					if ( ts.FFT_stand_by == 0 )
					{
						ts.FFT_sampling_enable      = true;
						ts.enable_display_on_screen = true;
						sd.samp_ptr                 = 0;
					}
				}

				else  if ( !ts.FFT_sampling_enable && ts.enable_display_on_screen )
				{
					freq_calcc  = RadioManagement_GetRXDialFrequency();      // get current tune frequency in Hz pour le d�calage graphique rapide du Spectre
		...
		case 4:
				{
					int32_t    cur_center_hzz      = freq_calcc;
					int32_t    diff_centerss       = ( cur_center_hzz  - last_cur_center_hz );
							   last_cur_center_hz  =   cur_center_hzz;
				    float32_t  normal_Hz_per_pixel = sd.hz_per_pixel * 1.25;
					int32_t    offset_pixel;
							   if (diff_centerss  >= 0) { offset_pixel = (diff_centerss + normal_Hz_per_pixel/2 - 1) /normal_Hz_per_pixel;}
							   else                     { offset_pixel = (diff_centerss - normal_Hz_per_pixel/2 + 1) /normal_Hz_per_pixel;}

											 ///*******************************************************
											 ///

				/*	int32_t   demi_Length = sd.spec_len/2 ;
					if (offset_pixel !=0)
					{
						if ((offset_pixel <  demi_Length -1) && (offset_pixel > -demi_Length+1))
						{
							if (offset_pixel >0)
							{
								for (int32_t jj=(demi_Length -1); jj>=offset_pixel; jj--)
								{
									sd.FFT_AVGData[jj] = sd.FFT_AVGData[jj - offset_pixel];
								}
								int32_t  sheeft = sd.spec_len - offset_pixel;
								for (int32_t jj =(offset_pixel -1); jj>=0; jj--)
								{
									sd.FFT_AVGData[jj] = sd.FFT_AVGData[ jj + sheeft ];
								}

								for (int32_t jj=(sd.spec_len -1); jj >= (demi_Length + offset_pixel); jj--)
								{
									sd.FFT_AVGData[jj] = sd.FFT_AVGData[jj - offset_pixel];
								}

								float32_t cumul = 0;
								for (int32_t jj=demi_Length ; jj <= (demi_Length + offset_pixel); jj++)
								{
									if (sd.FFT_MagData[jj]>50) { if (sd.FFT_MagData[jj]>300) { cumul+=300;} else { cumul += sd.FFT_MagData[jj];}} else { cumul += 50;}
								}
								cumul /= offset_pixel;

								for (int32_t jj=demi_Length ; jj <= (demi_Length + offset_pixel); jj++)
								{
									sd.FFT_AVGData[jj] =  cumul ;
								}
							}
							else
							{

								for (int32_t jj=demi_Length; jj <= (sd.spec_len -1 + offset_pixel); jj++)
								{
									sd.FFT_AVGData[jj] = sd.FFT_AVGData[jj - offset_pixel];
								}

								int32_t sheeft = sd.spec_len + offset_pixel;
								for (int32_t jj=(sd.spec_len + offset_pixel); jj <= (sd.spec_len -1) ; jj++)
								{
									sd.FFT_AVGData[jj] = sd.FFT_AVGData[jj - sheeft];
								}

								for (int32_t jj=0; jj<= (demi_Length -1 + offset_pixel);  jj++)
								{
									sd.FFT_AVGData[jj] = sd.FFT_AVGData[jj - offset_pixel];
								}
								float32_t cumul = 0;
								for (int32_t jj=(demi_Length  + offset_pixel); jj<= (demi_Length -1) ;  jj++)
								{
									if (sd.FFT_MagData[jj]>50) {if (sd.FFT_MagData[jj]>300) { cumul+=300;} else { cumul += sd.FFT_MagData[jj];}} else { cumul +=50;}
								}
								cumul /= (-offset_pixel);
						...
						{
							for (int ff=0; ff < sd.spec_len; ff++) { sd.FFT_AVGData[ff] = 50; }
						}

					}  */

					float32_t filt_factor   = 1/(float)ts.spectrum_filter;	       // use stored filter setting inverted to allow multiplication
					if      ( ts.switch_pause > 1450) { filt_factor  =  0.0004; }  // pour ne pas moyenner le spectre juste au moment de retour vers Rx
					else if ( ts.tempo_avg_FFT      ) { filt_factor  =  0.125;  }  // pour moyennage rapide en cas de touchescreen sur spectre
					int32_t  Length_spec      =  sd.spec_len;
					int32_t  demi_Length_spec =  Length_spec/2 ;

					if ( offset_pixel != 0 )      /// NIZZZ  pour assurer le d�calage fr�quenciel rapide du graphique du FFT-Magn
					{
						if (ts.switch_pause    < 400) ts.switch_pause += 200; // pour suspendre momentan�ment le calcul non prioritaire de reajustement des correction IQ, besoin du processeur
						if ( abs(offset_pixel) < Length_spec*3/4)
						{
							int32_t  aa =  demi_Length_spec - 1;

							if (offset_pixel < 0)
							{
								offset_pixel = - offset_pixel;
								int32_t   bb =    demi_Length_spec - offset_pixel;
								for (int  ii = 0; ii < Length_spec - offset_pixel; ii++ )
								{
									int   j  = -demi_Length_spec + ii;              // int j = -demi_Length_spec + ii
									int   k  =  j + offset_pixel;
									if   (ii <  demi_Length_spec )
									{
													 j += Length_spec;
										if (ii < bb) k += Length_spec;
									}
									sd.FFT_AVGData[j] = sd.FFT_AVGData[k];
								}

								float32_t cumul  = 0;

								int       limits = offset_pixel;
								if      ( limits > 70) { limits = 70; }
								int          deb = 0;
								if (sd.magnify == 1 ) { deb = 2; if ( limits < 5) { limits =  5; }}
								else                  {          if ( limits < 4) { limits =  4; }}
								for (int ii = deb; ii < limits; ii++ )  ///estimer la valeur de remplissage de intervalle vide
								{
									float32_t magn = sd.FFT_MagData[aa - ii];
									if (magn>25) {if (magn>350) { cumul+=350;} else { cumul += magn;}} else { cumul +=25;}
								}
								cumul /= (limits - deb);
								for (int ii=0; ii < offset_pixel; ii++ )  /// remplir le nouveau intervalle vide
								{
								   int j    = aa - ii;
								   if ( ii >= demi_Length_spec) { j += Length_spec; }
								   sd.FFT_AVGData[j] = cumul;
								}
							}
							else
							{
								...
								for (int ii= deb; ii < limits; ii++ )  /// remplir le nouveau intervalle vide
								{
									float32_t  magn  = sd.FFT_MagData[ demi_Length_spec + ii];
									if (magn>25) { if (magn>350) { cumul+=350;} else { cumul += magn;}} else { cumul += 25;}
								}
								cumul /= (limits - deb);
								...
								for (int ii=0; ii < offset_pixel; ii++ )  /// remplir le nouveau intervalle vide
								{
								   int             j  =  cc + ii;    if (ii < demi_Length_spec) { j += Length_spec; }
								   sd.FFT_AVGData[j]  = cumul;
								}
							}
						}
						else   // saut de spectrum entier
						{
							int32_t     quard = Length_spec/4;
							float32_t   cumul = 0;
							for ( int ii = quard ; ii < Length_spec - quard; ii++ )  /// remplir le nouveau intervalle vide
							{
								float32_t  magn  = sd.FFT_MagData[ ii ];
								if (magn>25) { if (magn>350) { cumul+=350;} else { cumul += magn;}} else { cumul += 25;}
							}
							cumul /= demi_Length_spec;

							for ( int ii=0; ii < Length_spec; ii++ )  /// remplir le nouveau intervalle vide
							{
								sd.FFT_AVGData[ii]  = cumul;
							}
						}
					}
					...
					if ( ts.lcd_backlight_brightness != 0 ) //&& (ts.lcd_backlight_brightness != 5)) /// pour masquer le DC Spike lorsque la luminosité de l'ecran est réduite
					{
						switch (sd.magnify)            ///  NIZZZ pour reduire le DC Spike au centre du spectre pour Zoom1X et Zoom2X
						{
							case  0 :   // Magnify X1
								sd.FFT_AVGData [                  0] = sd.FFT_AVGData [                  3];
								sd.FFT_AVGData [                  1] = sd.FFT_AVGData [                  3];
								sd.FFT_AVGData [     Length_spec -1] = sd.FFT_AVGData [     Length_spec -4];
								sd.FFT_AVGData [     Length_spec -2] = sd.FFT_AVGData [     Length_spec -4];
								break;
							case  1 :  // Magnify X2
								sd.FFT_AVGData [demi_Length_spec -2] = sd.FFT_AVGData [demi_Length_spec -4];
								sd.FFT_AVGData [demi_Length_spec -1] = sd.FFT_AVGData [demi_Length_spec -4];
								sd.FFT_AVGData [demi_Length_spec   ] = sd.FFT_AVGData [demi_Length_spec +3];
								sd.FFT_AVGData [demi_Length_spec +1] = sd.FFT_AVGData [demi_Length_spec +3];
								break;
						}
					}
					...
					{
						if( sd.FFT_AVGData[i] < 1 )	{ sd.FFT_AVGData[i] = 1; }
					}

					// else arm_fill_f32( 150  , sd.FFT_Samples, sd.spec_len);

					  // arm_scale_f32(sd.FFT_MagData, 0.2  , sd.FFT_Samples, sd.spec_len);
		...
		case 5:
				if ( ( ts.dmod_mode != DEMOD_USB ) && ( ts.dmod_mode != DEMOD_LSB) /*&& ( ts.dmod_mode != DEMOD_CW )*/ )  UiSpectrum_CalculateDBm();
		...
		case 6:	// rescale waterfall horizontally, apply brightness/contrast, process pallate and put vertical line on screen, if enabled.
				if (is_RedrawActive)		//this is needed for overwrite prevention if menu was drawn when sd.state>4
				{
					if ( sd.RedrawType & Redraw_SCOPE    )  {	UiSpectrum_DrawScope( sd.Old_PosData, sd.FFT_Samples);}
					if ( sd.RedrawType & Redraw_WATERFALL)  {	UiSpectrum_DrawWaterfall();	                          }
					if ( ((ts.waterfall.speed   == 0) && (ts.scope_speed == 0)) || (sd.RedrawType  != 0) ) { sd.state = 0; ts.FFT_sampling_enable = true;}
				}
				else {	sd.state = 0; }
				sd.RedrawType = 0;
				break;
...
//***********************************************************************************
/*
int32_t  UISpectrum_track_optimal_USB_Tune( int32_t tune_position_diff )
{
	int32_t    index_max    ; index_max   = 0 ;
	int32_t    decal_fact   ; decal_fact  = ts.beep_loudness;
	int32_t    mm           ; mm          = ts.dsp_notch_mu;
	int32_t    Length       ; Length      = sd.spec_len   ; // 256
	int32_t    demi_Length  ; demi_Length = sd.spec_len/2 ; // 128
	int32_t    index_ini    ;
	int32_t    index_sec    ;
	float32_t  val_max      ; val_max     = 0;
	int32_t    tune_position;


	switch(ts.iq_freq_mode)
	{
		case FREQ_IQ_CONV_M12KHZ  :	tune_position  =  demi_Length + Length/4;   break;
		case FREQ_IQ_CONV_P12KHZ  :	tune_position  =                Length/4;   break;
		case FREQ_IQ_CONV_M6KHZ   : tune_position  =  demi_Length + Length/8;   break;
		case FREQ_IQ_CONV_P6KHZ   :	tune_position  =  demi_Length - Length/8;   break;
		case FREQ_IQ_CONV_MODE_OFF: tune_position  =  demi_Length           ;   break;
	}

	if (ts.freq_step_config & FREQ_STEP_SWAP_BTN)  { tune_position -= 1 ;}	// if Step Button swap "ON"
	                                                 tune_position += tune_position_diff;
	...
	switch (sd.magnify)
	{
		case  0 :  peak_offset = 1;  break; // Magnify X1
		case  1 :  peak_offset = 2;  break; // Magnify X2
		case  2 :  peak_offset = 4;  break; // Magnify X4
		case  3 :  peak_offset = 8;  break; // Magnify X8
	}

	for ( int ii = -mm;  ii<= mm; ii++ )
	{
		int32_t  jj = tune_position + ii  ;
		if (jj < demi_Length) { index_ini =           demi_Length  - 1 - jj;}
		else                  { index_ini =  Length + demi_Length  - 1 - jj;}


		int32_t  kk = jj + peak_offset;
		if (kk < demi_Length) { index_sec =           demi_Length  - 1 - kk;}
		else                  { index_sec =  Length + demi_Length  - 1 - kk;}


		if ( (kk>=0) && ( kk < Length)&& (jj>=0) && (jj< Length) && (index_sec >= 0) &&  (index_sec < Length) && (index_ini < Length) && (index_ini >= 0))
		{
			float32_t  differ = sd.FFT_AVGData[index_ini] - sd.FFT_AVGData[index_sec] ;
			if (differ >  val_max) { val_max = differ;  index_max = ii;  }
		}
	}

	return   4* ( tune_position_diff + index_max ) -  decal_fact * peak_offset ;
}
*/
//***********************************************************************************
//***********************************************************************************
//***********************************************************************************
/*
int32_t  UISpectrum_track_optimal_LSB_Tune( int32_t tune_position_diff )
{
	int32_t    index_max    ; index_max   = 0 ;
	int32_t    decal_fact   ; decal_fact  = ts.beep_loudness;
	int32_t    mm           ; mm          = ts.dsp_notch_mu;
	...
	int32_t    index_sec    ;
	float32_t  val_max      ; val_max     = 0;
	int32_t    tune_position; // from 0..255  // -127..0..+127  on spectrum
	...
	if (ts.freq_step_config & FREQ_STEP_SWAP_BTN)  { tune_position -= 1 ;}	 // Step Button swap "ON"
	                                                 tune_position += tune_position_diff  ;
	AudioManagement_KeyBeep();
...
int32_t  UISpectrum_track_optimal_SSB_Tune( int32_t tune_position_diff )      ///  NIZZZ pour recherche du point graphic le plus optimal lors d'un touchscreen sur spectre pour demod BLU
{
	int32_t    index_max     = 0 ;
	int32_t    Length        = sd.spec_len ;  // 256
	int32_t    demi_Length   = Length >> 1 ;  // 128
	float32_t  val_max       = 0;
	int32_t    tune_position = demi_Length;   // from 0..255  // -127..0..+127  on spectrum
	int32_t    peak_offset   = 0;
	int32_t    decal_fact    = 0;             //  ts.beep_loudness;

	if ((ts.dmod_mode  == DEMOD_USB ) || (ts.dmod_mode  == DEMOD_LSB )|| (ts.dmod_mode  == DEMOD_CW ))
	{
		/// AudioManagement_KeyBeep();   /// masqu� pour ne pas perturber les traitements intenses en cours
		val_max       = 0;
	 // decal_fact    = 0;

		if (sd.magnify == 0)  // seulement pour le mode Zoom  X1 , car le curseur n'y est pas centr�
		{
			switch(ts.iq_freq_mode)
			{
				case FREQ_IQ_CONV_M12KHZ  :	tune_position  +=  (Length >> 2);   break;  // +1/4
				case FREQ_IQ_CONV_P12KHZ  :	tune_position  -=  (Length >> 2);   break;  // -1/4
				case FREQ_IQ_CONV_M6KHZ   : tune_position  +=  (Length >> 3);   break;  // +1/8
				case FREQ_IQ_CONV_P6KHZ   :	tune_position  -=  (Length >> 3);   break;  // -1/8
			}
		}
		tune_position += tune_position_diff - 1  ; // le -1 est confirm� difinitif , on en parle plus
		if ( ts.dmod_mode  !=  DEMOD_CW )
		{
			peak_offset = ( 1 << sd.magnify );  /// puissance de 2 X Zoom Magnify
			if ( ts.dmod_mode == DEMOD_LSB )  peak_offset = - peak_offset;
			decal_fact = 10 ;  //  ts.beep_loudness;
		}
		else { peak_offset = 1; }
		...
		int32_t    index_thi    ;
		int32_t    index_qua    ;
		int32_t    DL          = demi_Length - 1;
		int32_t    TDL         = DL + Length;
		for ( int ii = -4;  ii<= 4; ii++ )    /// 7 points de recherche d'optimisation en coordonn�es pixel
		{
			int32_t  jj = tune_position + ii  ;
			if (jj < demi_Length) { index_ini =   DL   - jj;}
			else                  { index_ini =  TDL   - jj;}


			jj += peak_offset;
			if (jj < demi_Length) { index_sec =   DL  - jj;}
			else                  { index_sec =  TDL  - jj;}


			jj += peak_offset;
			if (jj < demi_Length) { index_thi =   DL  - jj;}
			else                  { index_thi =  TDL  - jj;}
		...
		// 	if ( (index_ini>=0) && (index_ini<Length) && (jj>=0) && (jj< Length) && (ll >= 0) && (ll<Length) && (mm<Length) && (mm>= 0))
			{
				float32_t  differ;
				if (ts.dmod_mode  != DEMOD_CW )
				{
					jj += peak_offset;
					if (jj < demi_Length) { index_qua =   DL  - jj;}
					else                  { index_qua =  TDL  - jj;}
					differ  = +sd.FFT_AVGData[index_qua] - sd.FFT_AVGData[index_sec]
							  -sd.FFT_AVGData[index_ini] + sd.FFT_AVGData[index_thi] ;

				    if ( differ >  val_max )    // recherche du maxi
					{
						val_max    = differ;
						index_max  = ii ;
					}
				...
				{
					differ   = sd.FFT_AVGData[index_sec];

					if ( differ >  val_max )    // recherche du maxi
					{
						val_max     = differ;
						index_max   = ii;
						decal_fact  = 16 + 14 * ( sd.FFT_AVGData[index_thi] - sd.FFT_AVGData[index_ini] ) / sd.FFT_AVGData[index_sec];
					}
				}
			}
		}
	}
	return   16 * ( tune_position_diff + index_max ) +  decal_fact * peak_offset;
}
///**********************************************************************************
/*

int32_t  UISpectrum_track_optimal_SSB_Tune( int32_t tune_position_diff )
{
	int32_t   index_max  ;
	int32_t   peak_offset;
	int32_t   decal_fact ;
	...
	{
		int32_t   Length         =  sd.spec_len   ; // 256
		int32_t   demi_Length    =  sd.spec_len/2 ; // 128
		int32_t   tune_position  ; //  =  Length - Length/4 -1 + tune_position_diff;
		switch(ts.iq_freq_mode)
		{
			case FREQ_IQ_CONV_M12KHZ  :	tune_position  =  demi_Length + Length/4;   break;
			case FREQ_IQ_CONV_P12KHZ  :	tune_position  =                Length/4;   break;
			...
			case FREQ_IQ_CONV_MODE_OFF: tune_position  =  demi_Length           ;   break;
									...
									/// le -1 vient du phenom�ne du bord des indices

		if (ts.freq_step_config & FREQ_STEP_SWAP_BTN)
		{
			zero_offset = -2 ;	  // Step Button swap "ON"
		}
		tune_position += tune_position_diff + zero_offset ;
	 ...
	 // int32_t    index_thir;
	    ...
	    if ((ts.dmod_mode == DEMOD_LSB ) || ((ts.dmod_mode == DEMOD_CW) && ts.cw_lsb))
		{
	    	AudioManagement_KeyBeep();
			peak_offset = - peak_offset;
		}
                 decal_fact  =  ts.beep_loudness;
        int32_t  mm          =  ts.dsp_notch_mu;
		...
		for ( int ii = -mm;  ii<= mm; ii++ )
		{
			...
			if (  (index_sec >= 0) &&  (index_sec < Length) && (index_ini < Length) && (index_ini >= 0))
			{
				float32_t  differ = sd.FFT_AVGData[index_ini] - sd.FFT_AVGData[index_sec] ; // - sd.FFT_AVGData[index_thir];
...
{
	if (ts.switch_pause < 300)  ts.switch_pause += 300; // pour suspendre momentanement les calculs correc IQ
	// if ( ts.scope_scheduler     < 273 )  ts.scope_scheduler     = 273;   // pour retarder l'affichage du spectre
	// if ( ts.waterfall.scheduler < 251 )  ts.waterfall.scheduler = 251;   // pour retarder l'affichage du spectre
...
void UiSpectrum_DisplayFilterBW()
{
    ...
    {// bail out if in menu mode
        ...
        uint16_t   pos_bw_y = slayout.graticule.y + slayout.graticule.h - 3 ;
    ...
    if (ts.spectrum_freqscale_colour != SPEC_BLACK)     // don't bother updating frequency scale if it is black (invisible)!
    {
       ...
       // FIXME: This code expect 8 vertical lines)
        for (int idx = -4; idx < 5; idx += (sd.magnify < 2) ? 1 : 2 )
        {
            ...
            const uint16_t pos_number_y = 1 + (slayout.graticule.y +  (slayout.graticule.h - UiLcdHy28_TextHeight(graticule_font))/2)  ;
            if (idx != centerIdx)
            {
                char *c;
                if(sd.magnify < 3)     /// NIZZZ
                {
                    snprintf(txt,16, "%03lu", ((uint32_t)(freq_calc+(idx*grat)))%1000);   // build string for middle-left frequency (1khz precision)
                ...
                {
                    UiLcdHy28_PrintText( slayout.graticule.x + pos    , pos_number_y - 2,c,clr,Black,graticule_font);
                }
                else if (idx == 4) // right border
                {
                    UiLcdHy28_PrintTextRight( slayout.graticule.x + pos   , pos_number_y -2 ,c,clr,Black,graticule_font);
                }

                else
                {
                    UiLcdHy28_PrintTextCentered(slayout.graticule.x +  pos - number_width/2 ,pos_number_y -2, number_width,c,clr,Black,graticule_font);
                }
            }
        }
    }
...
void UiSpectrum_Redraw()  // appel� chaque 10 ms  pour calcul FFT  si buffer 256/512 est rempli
{
    // Only in RX mode and NOT while powering down or in menu mode or if displaying memory information
	if      (
				  // (ts.txrx_mode == TRX_MODE_RX)  // condition deja ajout�e � l'appelle de la fonction
				  //&& (ts.menu_mode == false)
				  //	&&
			...
			)
    {
        ...
        if(ts.scope_scheduler == 0 && is_scopemode())   // is spectrum mode enabled?
        {
...
static void  UiSpectrum_DisplayDbm()
{
    ...
    if( ts.txrx_mode == TRX_MODE_RX)
    {
        ...
        {
			case DISPLAY_S_METER_DBM  : display_something = true;	val = sm.dbm;   unit_label = "dBm   ";	break;
			case DISPLAY_S_METER_DBMHZ: display_something = true;	val = sm.dbmhz;	unit_label = "dBm/Hz";	break;
        }
...
void UiSpectrum_InitCwSnapDisplay (bool visible)
{
	...
	if(!visible)
	{
        ...
        UiLcdHy28_DrawFullRect(ts.Layout->SNAP_CARRIER.x-27, ts.Layout->SNAP_CARRIER.y-13, 6+13, 59, Black); // NIZZZ pour effacer l'indication de decalage de freq
	}
	//Horizontal lines of box
	UiLcdHy28_DrawStraightLine(ts.Layout->SNAP_CARRIER.x-27, ts.Layout->SNAP_CARRIER.y + 6,  27, LCD_DIR_HORIZONTAL, color);
	UiLcdHy28_DrawStraightLine(ts.Layout->SNAP_CARRIER.x+5,  ts.Layout->SNAP_CARRIER.y + 6,  27, LCD_DIR_HORIZONTAL, color);
	UiLcdHy28_DrawStraightLine(ts.Layout->SNAP_CARRIER.x-27, ts.Layout->SNAP_CARRIER.y - 1,  27, LCD_DIR_HORIZONTAL, color);
	UiLcdHy28_DrawStraightLine(ts.Layout->SNAP_CARRIER.x+5,  ts.Layout->SNAP_CARRIER.y - 1,  27, LCD_DIR_HORIZONTAL, color);
	// vertical lines of box
	UiLcdHy28_DrawStraightLine(ts.Layout->SNAP_CARRIER.x-27, ts.Layout->SNAP_CARRIER.y - 1,  8,  LCD_DIR_VERTICAL,   color);
	UiLcdHy28_DrawStraightLine(ts.Layout->SNAP_CARRIER.x+31, ts.Layout->SNAP_CARRIER.y - 1,  8,  LCD_DIR_VERTICAL,   color);
}
...
#define  alpha       0.7
	...
	{
	    UiLcdHy28_DrawStraightLineDouble( ts.Layout->SNAP_CARRIER.x + old_delta_p + 1,  ts.Layout->SNAP_CARRIER.y,
																						6,
																						LCD_DIR_VERTICAL,
																						Black);

	    UiLcdHy28_DrawStraightLineDouble( ts.Layout->SNAP_CARRIER.x + delta_p + 1,	ts.Layout->SNAP_CARRIER.y,
																					6,
																					LCD_DIR_VERTICAL,
																					Yellow);
...
void UiSpectrum_CalculateSnap(float32_t Lbin, float32_t Ubin, int posbin, float32_t bin_BW)
{
	...
	float32_t help_freq = (float32_t) df.tune_old ;
...
//  delta2 = ( bin_BW * (1.75 * (bin3 - bin1)) / (bin1 + bin2 + bin3));
    ...
    {
    	if(ts.digi_lsb)	{ delta = delta + PSK_OFFSET; }
    	else    	    { delta = delta - PSK_OFFSET; }
    }
...
static  void  UiSpectrum_CalculateDBm() // pour calculer et puis afficher le Smeter en dehors des modes SSB  et en m�me temps afficher le dbm
{
    ...
    //
    {
        if( ts.txrx_mode == TRX_MODE_RX)
        {
            ...
            const  float32_t   cons   =  ts.dbm_constant - 225 - (sd.fft_iq_len == 1024?3:0);  /// NIZZZ
            ...
            if(sd.magnify == 0)
            {
                if      (ts.iq_freq_mode == FREQ_IQ_CONV_P6KHZ )  { bin_offset = -(buff_len_int / 16); } // we are in RF LO HIGH mode (tuning is below center of screen)
                else if (ts.iq_freq_mode == FREQ_IQ_CONV_M6KHZ )  { bin_offset =  (buff_len_int / 16); } // we are in RF LO LOW mode (tuning is above center of screen)
                else if (ts.iq_freq_mode == FREQ_IQ_CONV_P12KHZ)  { bin_offset = -(buff_len_int / 8 ); } // we are in RF LO HIGH mode (tuning is below center of screen)
                else if (ts.iq_freq_mode == FREQ_IQ_CONV_M12KHZ)  { bin_offset =  (buff_len_int / 8 ); } // we are in RF LO LOW mode (tuning is above center of screen)
            }
            ...
            if (ts.dmod_mode != DEMOD_CW )
            {
				...
				if (sum_db > 0)
				{
					sm.dbm   =        slope * log10f_fast (sum_db) + cons ; // - 77 ;  // slope* = 19.8    NIZZZ
	   ...
	   */
			 ...
			 // m_AttackAvedbm   = 0.40 * m_AttackAvedbm   +  0.60 * sm.dbm;
				m_AttackAvedbm   =                                   sm.dbm;
				m_DecayAvedbm    = 0.70 * m_DecayAvedbm    +  0.30 * sm.dbm;
				m_AttackAvedbmhz = 0.50 * m_AttackAvedbmhz +  0.50 * sm.dbmhz;
				m_DecayAvedbmhz  = 0.95 * m_DecayAvedbmhz  +  0.05 * sm.dbmhz;
				...
				//        long dbm_Hz = (long) m_AverageMag;      ts.Sm_peak_moyen
				if (ts.dmod_mode != DEMOD_CW )
				{
					sm.dbm      = m_AverageMagdbm ; // write average into variable for S-meter display
				}

				//sm.dbm    =  35 * log10f_fast (ts.Sm_peak_moyen) + cons - ts.dbm_constant * 10  ;  // slope* = 19.8    NIZZZ
				sm.dbmhz    =  m_AverageMagdbmhz; // write average into variable for S-meter display

		   /*     char  outxtt[10];
				snprintf ( outxtt, 10, "%6d",(int) sm.dbm );
				UiLcdHy28_PrintText ( 0, 100, outxtt, White,  Black, 4 ); */
            }
        }
