...
#define W8731_ANLG_AU_PATH_CNTR         0x04        // 0000100    //  Path Control
#define W8731_DIGI_AU_PATH_CNTR         0x05        // 0000101    //  path Control
#define W8731_POWER_DOWN_CNTR           0x06        // 0000110    //  power down control
...
#define W8731_ANLG_AU_PATH_CNTR_MUTE_MIC    (0x02)
#define W8731_ANLG_AU_PATH_CNTR_MICBBOOST   (0x01)
#define W8731_DIGI_AU_INTF_FORMAT_PHILIPS    0x02           //  0x02    left justified au lieu de left-1
...
#define W8731_DIGI_AU_INTF_FORMAT_24B       (0x02 << 2)
#define W8731_DIGI_AU_INTF_FORMAT_32B       (0x03 << 2)               // (0x11 << 2)  // NIZ32

#define W8731_DIGI_AU_INTF_FORMAT_I2S_PROTO W8731_DIGI_AU_INTF_FORMAT_PHILIPS

#define W8731_POWER_DOWN_CNTR_POWER_OFF  (0x80)
...
static uint32_t Codec_ResetCodec ( I2C_HandleTypeDef* hi2c, uint32_t AudioFreq, uint32_t word_size)
{
    ...
    if( retval == HAL_OK)
    {
        ...
        Codec_WriteRegister(hi2c, W8731_ANLG_AU_PATH_CNTR,  W8731_ANLG_AU_PATH_CNTR_DACSEL     |
                											W8731_ANLG_AU_PATH_CNTR_INSEL_LINE |
															W8731_ANLG_AU_PATH_CNTR_MUTE_MIC    );
     ...
     // uint16_t  size_reg_val = word_size == WORD_SIZE_16? W8731_DIGI_AU_INTF_FORMAT_16B : W8731_DIGI_AU_INTF_FORMAT_32B;
        uint16_t  size_reg_val = W8731_DIGI_AU_INTF_FORMAT_32B;    // NIZ32

     // uint16_t  size_reg_val = W8731_DIGI_AU_INTF_FORMAT_24B;    // Niz24
        ...
        switch ( AudioFreq )
        {
			case I2S_AUDIOFREQ_32K:       samp_reg_val = 0x0018;     break;
			case I2S_AUDIOFREQ_96K:       samp_reg_val = 0x001C;     break;
			case I2S_AUDIOFREQ_8K :       samp_reg_val = 0x000C;     break;
			case I2S_AUDIOFREQ_48K:
			default:             		  samp_reg_val = 0x0000;     break;
        }
        ...
        Codec_WriteRegister ( hi2c, W8731_ACTIVE_CNTR, 0x0001 ); // nizou D�marrage  du Codec
    }
    return retval;
...
uint32_t  Codec_Reset ( uint32_t  AudioFreq,  uint32_t word_size )
{
    ...
    if (retval == 0)
    {
        ...
        //  non_os_delay();   //  NIZZZ   ajout�
        Codec_Volume_Line_Out ( ts.txrx_mode ); // configure lineout according to mode
 ...
 */
void Codec_RestartI2S()   ///  NIZZZ
{
							    // if (ks.button_id == 17 ) AudioManagement_KeyBeep(); // if touchscreen then Beep
    Codec_WriteRegister ( CODEC_IQ_I2C, W8731_ACTIVE_CNTR, 0x0000 ); // Stop Codec    // Reg 09: Active Control
    non_os_delay();  non_os_delay();			// we can't use HAL_Delay here, since our audio interrupt has higher priority which stops the ticks.
    Codec_WriteRegister ( CODEC_IQ_I2C, W8731_ACTIVE_CNTR, 0x0001 ); // Re-Start Codec   // Reg 09: Active Control
   //  ts.old_rfg_calc = 100;      // elle est   n�cessaire si non le codec Rx va rester mute comme en Tx
}
...
void Codec_SwitchMicTxRxMode ( uint8_t txrx_mode )
{
    ...
    if( (txrx_mode == TRX_MODE_TX) && (ts.tx_audio_source == TX_AUDIO_MIC) )
    {
       ...
       // if(ts.tx_gain[TX_AUDIO_MIC] > 50)	 		// actively adjust microphone gain and microphone boost
    	if ( ts.codec_mic_boost_enable)   // if 20db codec gain was enabled
        {
            Codec_WriteRegister ( CODEC_ANA_I2C, W8731_ANLG_AU_PATH_CNTR, 	W8731_ANLG_AU_PATH_CNTR_DACSEL    |
												 	 	 	 	 	 	 	W8731_ANLG_AU_PATH_CNTR_INSEL_MIC |
																			W8731_ANLG_AU_PATH_CNTR_MICBBOOST   ); // mic boost on  +20db Hardware
            // ts.tx_mic_gain_mult = (ts.tx_gain[TX_AUDIO_MIC] - 35)/3;			// above 50, rescale software amplification
        }
        else
        {
            Codec_WriteRegister( CODEC_ANA_I2C, 	W8731_ANLG_AU_PATH_CNTR,  W8731_ANLG_AU_PATH_CNTR_DACSEL
            																| W8731_ANLG_AU_PATH_CNTR_INSEL_MIC );	// mic boost off  0db
...
{
	Codec_Line_In_Gain_Adj(0); // yes - momentarily mute LINE IN audio if in LINE IN mode until we have switched to TX
	...
	{
		Codec_VolumeSpkr(0);
		Codec_Volume_Line_Out( TRX_MODE_TX); // yes - mute the audio codec to suppress an approx. 6 kHz chirp when going in to TX mode
		ts.last_volume =  0 ;
	}
...
{
    			//*************** First step - mute sounds *****************************
	if ( ts.dmod_mode != DEMOD_CW )
	{
		Codec_VolumeSpkr(0);                   // le HP Left est Mute dans tous les cas, le gain sera revisé auto/10ms en Rx seulement
		Codec_Volume_Line_Out ( txrx_mode );   // HP Right side (Rig Line-Out) : if(Tx) Mute , if(Rx) Set the Line-Out gain
	}
    ...
    {
						//*********************************************************************************************************************
						//************************** Mute Codec_Line inputs,  Both Rx Codec LIne_IN : Left & Right     ***************************
						//*********************************************************************************************************************
        Codec_Line_In_Gain_Adj(0);
						//*********************************************************************************************************************
						//**********************************  Mute Microphone *****************************************************************
						//*********************************************************************************************************************
						//********** Reg 04: Analog Audio Path Control (DAC sel, ADC line, Mute Mic)
    ...
    else		// prepare Tx transmit
    {
        ...
        {
                 if ( ts.tx_audio_source == TX_AUDIO_MIC )  { Codec_SwitchMicTxRxMode ( txrx_mode ); }  // activer le micro  avec ou sans boost
            else if ( ts.tx_audio_source != TX_AUDIO_DIG_EX || ts.tx_audio_source != TX_AUDIO_DIG_IQ )  // if in LINE in mode
            {
                 Codec_Line_In_Gain_Adj ( ts.tx_gain [ ts.tx_audio_source ] ); // set LINE input gain , routed via 4066 switch
            }
        }
    }
}
...
// Note that this function is called from places OTHER than Codec_RX_TX(), above!
    ...
    if(txrx_mode == TRX_MODE_TX)  		// bail out if not in transmit mode
    {
        ...
        if ( ts.cw_sidetone_gain )	 	// calculate if the sidetone gain is non-zero
        {
            ...
            if      (vcalc > 127)  { vcalc = 127; }			// enforce limits of calculation to range of attenuator
            else if	(vcalc <   0)  { vcalc =   0; }
        }
        Codec_VolumeSpkr      ( vcalc/2  );  // NIZZZ  divide by 5 to convert decibel to volume control steps
        Codec_Volume_Line_Out ( txrx_mode);	 // set the calculated sidetone volume
    }
}
...
void Codec_VolumeSpkr(uint8_t vol)    /// NIZZZ
{

#ifdef UI_BRD_MCHF
    uint32_t lv = vol*2 > W8731_VOL_MAX ?  W8731_VOL_MAX : vol*2;
...
void Codec_Volume_Line_Out (uint8_t txrx_mode)   // mettre a jour le niveau  du Line-out interface selon le Tx/Rx
{
    ...
    if ( (txrx_mode == TRX_MODE_TX) &&  ((ts.flags1& FLAGS1_MUTE_LINEOUT_TX) || (ts.iq_freq_mode && ts.dmod_mode != DEMOD_CW))   )
    {
 ...
 */
static void Codec_In_Gain_Adj (I2C_HandleTypeDef* hi2c, uchar gain)   // Both Codec LIne_IN : Left & Right
{
    uint16_t   l_gain;
 ...
 * @param gain in range of [0-255]
 */
void Codec_Line_In_Gain_Adj ( uchar gain )
{
    Codec_In_Gain_Adj ( CODEC_ANA_I2C, gain );
}
...
{
    Codec_In_Gain_Adj ( CODEC_IQ_I2C,  gain );
}
