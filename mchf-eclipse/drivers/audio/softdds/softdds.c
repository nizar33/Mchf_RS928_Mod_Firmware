...
void softdds_setFreqDDS(soft_dds_t* softdds_p, float32_t freq, uint32_t samp_rate,uint8_t smooth)
{
    ...
    if(smooth == false)    {  softdds_p->acc = 0;   }                  // Reset accumulator, if need smooth tone // transition, do not reset it (e.g. wspr)

    softdds_p->step = softdds_stepForSampleRate( freq , samp_rate );   // Calculate new step
}
